...
```{r}
#| code-summary: "R code for Hotelling's T2 test"
...
points(hypothesized_mean[1], hypothesized_mean[2], pch = 4, col = "red", cex = 1.5, lwd=2)

# Add the ellipse
alpha=0.05
Fvalue <- sqrt(p * (n - 1) / (n - p) * qf(1 - alpha, p, n - p))
lines(ellipse(S, centre = x_bar, level = 1-alpha, t=Fvalue/sqrt(n)), col = "blue", lwd = 2)

legend("topleft",
       legend = c("Data", "Sample Mean", "Hypothesized Mean", "95% Confidence Ellipse"), cex=0.8,
...
T2_limits <- data.frame(lower = sample_mean - T2_widths, upper = sample_mean + T2_widths)

# Calculate Bonferroni interval limits
Bonf_crit <- qt(1 - alpha / (2 * p), df = n - 1)
Bonf_widths <- Bonf_crit * sqrt(diag(S) / n)
Bonf_limits <- data.frame(lower = sample_mean - Bonf_widths, upper = sample_mean + Bonf_widths)

# Calculate points for the confidence ellipse
confidence_ellipse <- as.data.frame(ellipse::ellipse(S, centre = sample_mean, t = Fvalue /
                                                       sqrt(n)))
...
ggplot() +
  ...
  theme_bw() +
  coord_cartesian(xlim = c(4.85, 5.2), ylim = c(3.2, 3.7))
```
...
* It also controls the familywise error rate at $\alpha$. 
