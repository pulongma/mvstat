...
* The $T^2$ CI for the Halothane effect is shifted far above zero, indicating that the mean time between heartbeats is likely to be between `r signif(ci_T2$Lower[1],3)` and `r signif(ci_T2$Upper[1],3)` milliseconds longer when Halothane is used than that when Halothane is not used, averaging across the CO2 pressure levels.
* The $T^2$ CI for the CO2 effect is shifted below  0, indicating that the mean time between heartbeats is likely to be between `r abs(signif(ci_T2$Lower[2],3))` and `r abs(signif(ci_T2$Upper[2],3))` milliseconds shorter at high CO2 pressure than that at low CO2 pressure, averaging across the halothane effect.  
