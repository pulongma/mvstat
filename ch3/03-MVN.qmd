...
Given a $p \times p$ square matrix $A$, an eigenvector $\mathbf{v}$ and its corresponding eigenvalue $\lambda$ satisfy: 
...
* **Some Properties**
  ...
  - If $(\lambda_j, \mathbf{e}_j)$ is an eigenvalue-eigenvector pair for a $p \times p$ matrix $A$, by definition 
...
```{r}
#| code-summary: "R Code: Create a Matrix"
A = matrix(c(4, 2, 1, 3), nrow = 2)
``` 

```{r}
#| code-summary: "R Code: Spectral Decomposition"
eig = eigen(A)
print(eig)
```
...
```{r}
#| code-summary: "Matrix Determinant: `det` for Small  Matrices"
det(A) 
```

```{r}
#| code-summary: "Matrix Determinant: Eigen Decomposition"
E = eigen(A)
prod(E$values)
```
...
### R Exercise: MVN Properties

```{r}
#| code-fold: false
#| code-summary: "R Code: Simulate MVN"
mu = c(0,0)
...
```{r}
#| code-summary: "R Code: Bivariate Normal Density Contour"
library(mvtnorm)
library(ggplot2)
...
```{r}
#| code-summary: "R Code: 3D Surface Plot"
library(plotly)
...
```{r}
#| code-summary: "R Code: Covariance"
# compute covariance 
S <- cov(X)
print(S)
```
...
```{r}
#| code-summary: "R Code: Overall Measures"
...
```{r}
#| code-summary: "R Code: Univariate Normality Test"
...
```{r,message=FALSE, warning=FALSE}
#| code-summary: "Visualizing Data Using Scatterplot and Marginal Dot Plots"
library(ggplot2)
library(dplyr)
...
```{r}
#| code-summary: "Detecting Outliers Using Mahalanobis Distances"
...
```{r, message=FALSE, warning=FALSE}
#| code-summary: "Scatterplot and Dot Plot"
