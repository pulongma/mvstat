...
:::{.callout-note title="Plotting with `ggplot2` Package" collapse="true"}
```{r}
library(ggplot2)
data("USArrests")
...
- Introduction to Multivariate Data — *Chapter 1*
...
- Multidimensional Scaling — *Chapter 4*
...
:::{.callout-note title="R Code: Data Organization" collapse="true"}
...
:::{.callout-note title="Sample Covariance and Sample Standard Deviation"}
...
:::{.callout-note title="Sample Covariance and Sample Correlation"}
...
:::{.callout-note title="Correlation Measures Linear Association"}
...
p_corr <- ggplot(df, aes(x, y)) +
  ...
  labs(x = NULL, y = NULL,
       title = "Correlation shows linear association") +
...
:::{.callout-note title="Nonlinear Dependence with (Near) Zero Correlation"}
```{r, echo=FALSE}
#| fig-width: 5
#| fig-height: 3
#| fig.align: center
...
p_nonlin <- ggplot(nonlin, aes(x, y)) +
  ...
  labs(x = NULL, y = NULL,
       title = "Nonlinear dependence with (near) zero correlation") +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.minor = element_blank(),
    strip.text = element_text(face = "bold"),
...
:::{.callout-note title="Sample Covariance"}
...
:::{.callout-note title="Sample Correlation"}
...
### Standardized Data 
:::{.callout-note title="Standardized Data"}
...
:::{.callout-note title="Standardized Population Mean"}
...
:::{.callout-note title="Standardized Population Covariance"}
...
:::{.callout-tip title="Partial Solution" collapse="true"}

- `x` is binded to a vector of characters instead of            numerical values. The functions `max`, `sort`, `sum` should take numerical values as input, so the values in `x` are first converted from characters to numerical values implicitly and then the functions apply to these incorrect numerical values
...
### Exercise 3: Working with `ggplot2` Package
...
ggplot(df) + 
  geom_freqpoly(aes(x=PM2.5, y=after_stat(density), color=COUNTY), binwidth=2) + 
...
### Exercise 4: Working with `dplyr` Package
