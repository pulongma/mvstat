...
$$
  ...
  - **p-value**: p-value is less than $\alpha$. 
...
abline(lm(strength ~ yield, data = subset(steel, temp == 1)), lty = 4, lwd = 2)
abline(lm(strength ~ yield, data = subset(steel, temp == 2)), lty = 1, lwd = 2)
...
::: {.callout-note title="Visualization" collapse="true"}
```{r}
...
theme_bw()
```
The plot shows very clear separation between the three species, especially for the petal measurements. This gives us a strong indication that the null hypothesis will be rejected.
:::
...
::: {.callout-tip title="Check Assumptions" collapse="true"}
```{r}
#| label: check-assumptions-manova
#| code-summary: "R Code: Assumption Checks"
...
# check for Homogeneity of Covariance Matrices using Box's M-test.
...
::: {.callout-tip title="R Code for MANOVA Test" collapse="true"}
```{r}
#| label: run-manova
...
:::{.callout-tip title="R Code: Bonferroni CIs" collapse="true"}
...
df_summaries <- df %>%
  group_by(group) %>%
  dplyr::summarise(
    across(where(is.numeric), list(mean = mean, n = ~n())),
    .groups = "drop"
  )
...
all_intervals <- purrr::map_df(group_pairs, function(pair) {
  ...
  mean_diffs <- as.numeric(dplyr::select(summary1, 
                                         ends_with("_mean"))
                           ) - 
                as.numeric(dplyr::select(summary2, 
                                         ends_with("_mean")))
...
::: {.callout-note title="R Code: Testing Correlation" collapse="true"}
...
# 2. Calculate the OBSERVED correlation
