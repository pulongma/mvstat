...
2. $\Sigma_1=\Sigma_2=\Sigma$. (**Homogeneity of Covariance**)
...
::: {.callout-note title="Research Questions"}
...
:::{.callout-note title="Hypotheses" collapse="true"}
$$
...
### Pool Covariance
:::{.callout-note title="Pool Covariance"}
...
:::{.callout-note title="Hotelling's $T^2$ Statistic" collapse="false"}
...
:::{.callout-note title="Decision" collapse="false"}
...
:::{.callout-note title="Confidence Region" collapse="false"}
...
:::{.callout-note title="Simultaneous CIs"}
...
gg <- gg +
  # T2 CI limits
  geom_vline(
    data = filter(ci_lines, type == "T2", Component == "Lather"),
    aes(xintercept = value),
    linetype = "dashed",
    color = "blue"
  ) +
  geom_hline(
    data = filter(ci_lines, type == "T2", Component == "Mildness"),
  ...
  geom_vline(
    data = filter(ci_lines, type == "Bonferroni", Component == "Lather"),
    aes(xintercept = value),
    linetype = "dotted",
    color = "darkgreen"
  ) +
  geom_hline(
    data = filter(ci_lines, type == "Bonferroni", Component == "Mildness"),
    aes(yintercept = value),
    linetype = "dotted",
    color = "darkgreen"
  ) +
  annotate(
    "text",
    x = min(ci_Bon$Lower) - 0.2,
    y = max(ci_Bon$Upper) + 0.2,
    label = "Bonferroni CI",
    color = "darkgreen",
    hjust = 0
  ) +
  annotate(
    "text",
    x = min(ci_T2$Lower) - 0.2,
    y = max(ci_T2$Upper) + 0.2,
    label = "T² CI",
    color = "blue",
    hjust = 0
  )
...
```{r}
#| code-fold: true
#| code-summary: "R Code: Long Format via `pivot_longer`"
library(dplyr)
df_long = steel %>% 
...
```{r}
#| code-fold: true
#| code-summary: "R Code: One variable + multiple groups"
...
```{r}
#| code-summary: "R Code: One group + multiple variables"
#| code-fold: true
ggplot(steel) + 
...
## Comparing Multiple Mean Vectors
...
```{r}
#| code-summary: "R Code: Between Group plots"
library(dplyr)
library(ggplot2)
data(iris)
#head(iris)
...
```{r}
#| code-summary: "R Code: Between Group boxplots"
...
#| code-summary: "R Code: Pairwise Scatterplot"
#| message: false
#| warning: false
...
#| size: striptsize 
#| fig-align: center
#| fig-cap: 'Simultaneous 95% Bonferroni CIs. Intervals not crossing the dashed line (zero) are statistically significant.'
library(dplyr)
n = iris %>%
  count(Species) %>%
  pull(n, name = Species)
...
b. State the hypotheses using standard notations. 
...
### Exercise: Testing a Correlation
